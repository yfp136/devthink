...
2. `cmake --preset windows-msvc-debug`，**断言 configure 日志出现
   `FFmpeg found — enabling real media backend`**（出现 `stub media backend` 即失败退出）；
3. `cmake --build` + `ctest`，期望 **22/22 通过**（媒体真实分支随 sm_engines 一并编译链接）；
4. 启动 `sm_headless.exe --port 8099`：`GET /api/status` 必须 200（硬门槛）；
...
| 启动未见 `[audio]` 行 | 可能 stdout 全缓冲未落盘：进程退出后重看日志，或直接在终端前台运行（不重定向）；服务本身以 HTTP 200 为健康判据 |
...
- [ ] configure 日志出现 `FFmpeg found — enabling real media backend`
- [ ] `ctest` 22/22 全绿
- [ ] headless `GET /api/status` 200
