# ShowMaster 开发状态交接文档

日期：2026-09-10（更新版）
用途：把 ShowMaster 工程的代码布局、构建方式与当前完成状态交接给下一个开发任务。剩余工作请直接以 `docs/TASKS_REMAINING.md` 为执行清单；本文档只描述静态事实（位置、结构、纪律），不再承载“待办主线”。

> 文档纪律：本文档随代码变动同步更新。当本文档与 `docs/TASKS_REMAINING.md` 或源码不一致时，以源码为准，并立即修正本文档（属 H-1/H-2 类卫生项）。

## 1. 工程身份与位置

工程名 **ShowMaster**，对应《ShowMaster工程开发规格书 V2.0 可落地版》。语言标准 C++20，零第三方运行时依赖（仅随仓库分发 `nlohmann/json.hpp` 单头库，数据库用系统/平台 SQLite3）。

源码根目录：

```
/Users/yxiao88/Library/Application Support/TRAE SOLO CN/ModularData/ai-agent/work-mode-projects/6a9e6a7e48915cce9293e7ba/showmaster-src
```

规格书 docx 位于源码目录上一级（`ShowMaster工程开发规格书V2.0_可落地版.docx`），业务语义以它为最高依据。

## 2. 代码布局

| 构件 | 目录 | 职责 |
| --- | --- | --- |
| `sm_core`（静态库） | `src/core` `src/db` `src/project` | 信封/指令字典/错误码、消息总线与心跳、SHA-256/UUID/时间工具、SQLite 存储与迁移、工程清单、state_json |
| `sm_engines`（静态库） | `src/engines/*` `src/platform` | 引擎注册表 + 10 个引擎子目录（timeline/media/scene/playlist/media_lib/vjfx/led/light/pixel/device）+ kernel + 平台适配层（`media_backend.*`、`jpeg_codec.*`、`rhi/`） |
| `sm_web`（静态库） | `src/web` | HTTP/WebSocket 服务、认证、远程队列、网关、内嵌 HTML UI |
| `sm_remote`（静态库） | `src/remote` | Windows Service 管理、看门狗 |
| `sm_protocols`（静态库） | `src/protocols` | TCP / MIDI / OSC 适配器 |
| `sm_kernel_smoke`（exe） | `src/app/main.cpp` | 内核冒烟程序 |
| `sm_headless`（exe） | `src/app/main_headless.cpp` | Headless 服务器（工控机常驻、Web 远控入口）；主循环以 ~10Hz 驱动 PGM 预监推流 |
| `sm_desktop`（exe，可选） | `src/app/main_qt.cpp` `src/app/qt/` | Qt 6.7 + QML 桌面端（P1-1）：`kernel_host.{h,cpp}` 桥接、`ui_controller.{h,cpp}` 装配、`qml/Main.qml` 六面板 + `qml.qrc`；仅 `SM_BUILD_DESKTOP=ON` 时构建（默认 OFF） |
| 单元测试 | `tests/` | 每个用例一个独立可执行文件，断言失败打印 `[FAIL]` 并以非 0 退出 |
| 第三方 | `third_party/nlohmann/json.hpp` | 单头 JSON（MIT） |

Windows 专属：7 个引擎插件 DLL（MediaEngine/TimelineEngine/VjfxEngine/LedEngine/LightEngine/PixelEngine/DeviceEngine），由 `src/engines/plugins/engine_plugin.cpp` 模板生成，当前为 NoopEngine 空壳；真实功能在宿主静态库内实现。

## 3. 构建与测试

本机（macOS/Linux）开发循环：

```sh
cmake --preset debug
cmake --build --preset debug
ctest --preset debug
```

Windows：在 "x64 Native Tools Command Prompt for VS 2022" 中执行 `cmake --preset windows-msvc-debug -DCMAKE_TOOLCHAIN_FILE=%VCPKG_ROOT%\scripts\buildsystems\vcpkg.cmake`，随后 `--build` 与 `ctest` 同 preset；vcpkg 提供 `sqlite3` 与 `ffmpeg`。`find_package(FFMPEG)` 成功时定义 `SM_HAS_FFMPEG=1`，平台层走真实实现；找不到 FFmpeg 时自动降级为 stub。

当前实测：本机（macOS）`ctest` **26/26 全绿**，这是目前唯一可在本机复现的绿灯基线；`README.md`、`docs/BUILD_WINDOWS.md`、`docs/VERIFY_WINDOWS_MEDIA.md` 中的计数均已同步为 26/26。

Windows CI（`.github/workflows/windows-build.yml`）含两个 job：`build`（vcpkg `sqlite3`+`ffmpeg` → MSVC debug → `ctest` → headless 冒烟登录）与 `desktop`（aqt 预编译 Qt 6.7.2 → `SM_BUILD_DESKTOP=ON` Release → `windeployqt --qmldir` 部署 platforms/qml → `sm_desktop --smoke` 退出码断言）。**2026-09-10 实测：run #34434959091（commit `97760ad`）两个 job 全绿**；其中 `desktop` job 在后续 run（commit `1b17e97`）再次全绿，**首次真编译了 P1-2 的 D3D11 输出窗口代码**（步骤 8 Build sm_desktop / 10 Smoke test 均 success），即该部分已在 Windows/MSVC 下编译链接通过。**教训（同日）**：把 `sm_desktop` 输出视口**调用方接线**并入构建的 commit `f05e31b`（run #34437790755）在 `desktop` job 步骤 8 `Build sm_desktop` **失败**，报 `src/app/qt/output_window.cpp(60/82/105): error C2027: use of undefined type 'QDebug'` —— `<QString>/<QWindow>/<QtGlobal>` 只**前向声明** `QDebug`，而 `qWarning().noquote() <<` 需要完整类型（`ui_controller.cpp` 只因 `QQmlApplicationEngine` 传递包含才侥幸编译）；已补 `#include <QDebug>`。**已复验（commit `a94c26f` → run #34438302868）**：该 run 的 `desktop` job **全绿**（步骤 1–11 + 4 个 post 步骤全部 success，含步骤 8 `Build sm_desktop` 与步骤 10 `Smoke test sm_desktop (--smoke, expect exit code 0)`），即该修复在 Windows/MSVC 下确认有效；同 run 的 `build` job 于核对时仍在步骤 4 安装 vcpkg 依赖（`ffmpeg` 编译耗时较长）。**结论：本地无 Qt 环境时，「新增 Qt 翻译单元」的首次真编译风险只能靠 CI 兜住，故 `desktop` job 的绿灯必须逐次核对，不能沿用上一次 run 的结论。**后续绿灯状态请以 GitHub Actions 页面为准 —— 本机无 Qt/vcpkg 环境，无法本地复现 `desktop` job。

构建纪律（全仓强制）：MSVC 用 `/W4 /permissive-`，其他平台 `-Wall -Wextra`，新代码必须零告警；测试全绿才算完成。

新增测试必须在 `tests/CMakeLists.txt` 里调用一次 `sm_add_unit_test(name libs)`；新增 `src/platform/*.cpp` 由顶层 CMake 的 `file(GLOB ... CONFIGURE_DEPENDS)` 自动收集，无需改 CMake。

## 4. 平台层现状（2026-09-10 按源码核对）

### 4.1 `media_backend.h`（接口已定稿）

对外函数：`has_media_backend` / `probe_media` / `open_media_file` / `start_media_playback` / `stop_media_playback` / `get_media_pos_ms` / `get_media_duration_ms` / `close_media_file` / `generate_thumbnail`（JPEG base64）/ `capture_pgm_frame_jpeg`（JPEG base64）/ `init_audio_output` / `shutdown_audio_output`。

### 4.2 `media_backend.cpp`（实现现状）

文件按 `#if !defined(SM_HAS_FFMPEG)` 分成两支：

- stub 分支：macOS/Linux 编译，函数为空实现或返回空/`true`。
- FFmpeg 分支（Windows）：解码/播放/定位/关闭、后台解码线程、视频帧队列（保留最新一帧，RGB24，宽上限 1280）均已实现。
- 自研 JPEG 编码器 `sm::platform::encode_jpeg_rgb24`（`src/platform/jpeg_codec.cpp`）已实现并接线：`generate_thumbnail` 与 `capture_pgm_frame_jpeg` 均返回真实 JPEG base64，不再是空串。
- WASAPI 音频：`init_audio_output` 已做真实默认设备探测（`__uuidof(MMDeviceEnumerator)` / `IID_IAudioClient` 编译期 GUID），非占位。
- `capture_pgm_frame_jpeg` 的接口语义（`media_backend.h`）：有渲染上下文时返回渲染帧的 JPEG base64；**无渲染上下文（stub / 设备不可用 / 无帧）返回空串**。
- 实现（`media_backend.cpp` 尾部，规格 2.3 / P1-2）：在 `_WIN32 && SM_HAS_D3D11` 下走**渲染路径** —— 锁内拷贝队列最新帧 → RGB24 转 RGBA8 → `upload` 到 layer0（`BlendOp::Replace` 直通）→ 显式解绑 layer1–3（防残影）→ `compose_and_readback`（不透明黑底）→ 回读渲染帧编码 JPEG base64。已无“解码帧直出”旁路。
- 渲染后端不在本文件：抽象层与 D3D11 后端见 §4.3，`media_backend.cpp` 只作为 RHI 的 PGM 层调用方。

### 4.3 `rhi/`（渲染抽象层与 D3D11 后端）

- 接口 `src/platform/rhi/rhi.h`：`PixelFormat` / `BlendOp` / `BlendConfig`（`enable`/`op`/`opacity`/`mask_key`）/ `Texture2DDesc`、`ITexture`（`upload`/`readback`/`native_handle`）、`IPgmMixer`（`kMaxLayers=4`，`compose`/`compose_and_readback`）、`IDevice`，以及 P1-2 新增的输出窗口契约：`NdcRect` + `compute_ndc_rect()`（合成与窗口输出共用的等比居中/拉伸几何，纯头文件内联）、`OutputWindowDesc` + `is_valid_output_window_desc()`（`kMaxOutputDimension = 16384`）、`IOutputSurface`（`is_open`/`width`/`height`/`resize`/`present`/`native_handle`）、`IDevice::create_output_surface()`。RHI 只消费调用方传入的窗口句柄，自身不创建窗口、不跑消息循环。
- 工厂 `src/platform/rhi/rhi.cpp`：`create_device()` 在 `_WIN32 && SM_HAS_D3D11` 时返回 D3D11 设备，否则返回 `nullptr`（“无渲染上下文”语义，调用方据此返回空串）。
- 后端 `src/platform/rhi/rhi_d3d11.cpp`（+ `.h`）：双模式 ——（1）**离屏**设备（HW→WARP 回退），`UpdateSubresource` 上传，离屏渲染目标 + staging 纹理 `CopyResource`/`Map` 回读；（2）**本地窗口输出**（P1-2 落地）——在调用方 HWND 上创建 DXGI 交换链（`CreateSwapChainForHwnd`，`FLIP_DISCARD`/2 buffer 失败自动回退 `DISCARD`/1 buffer；`MakeWindowAssociation(DXGI_MWA_NO_ALT_ENTER)` 保证不改动调用方窗口，`OCCLUDED` 视为成功、设备丢失关闭输出面，`resize` 重建后缓冲且失败时按旧尺寸尽力恢复），`present` 以 vsync 收尾（1080p60 视口 → 60 Hz 上限），创建失败返回 `nullptr` 由调用方降级为“仅离屏 + 预监”。两条路径共用同一 `draw_texture_quad` + `compute_ndc_rect`，故预监与输出视口像素一致。混合由 blend state 实现（Replace/Over/Add/Multiply × opacity）；每层四边形按 `fit_center` 自适应。
- 构建接线：顶层 `CMakeLists.txt` 的 `file(GLOB ...)` 已纳入 `src/platform/rhi/*.cpp`；`if(WIN32)` 下 `sm_engines` PUBLIC 链接 `d3d11 dxgi d3dcompiler`、PRIVATE 定义 `SM_HAS_D3D11=1`（与 `SM_HAS_FFMPEG` 同模式；Windows SDK 自带这三个 lib，无需 `find_package`）。
- 调用方接线（P1-2 收尾，已完成）：桌面端 `src/app/qt/output_window.{h,cpp}` 是**唯一调用方**——把独立顶层 QWindow 的 `winId()` 与客户区尺寸经 `platform::open_output_window()` 登记给引擎，`widthChanged`/`heightChanged` 转发 `resize_output_window()`，关闭/析构调 `close_output_window()`。由 `UiController` 持有（声明顺序置于最后，故析构最先执行 `close()`，此时 worker 仍在运行）。交换链的建立/重建/每帧 `present(PGM 合成结果)` 全在引擎渲染所有者线程（= `KernelWorker` 捕获线程）内完成，**GUI 线程不出现任何 D3D/DXGI 类型**，两侧无共享 RHI 对象（`IDevice` 及其产物非线程安全，单一所有者）。刻意使用**独立窗口**而非挂在 1600×900 六区 QML 主窗口上：DXGI `Present` 会覆盖整个客户区，挂主窗口会把界面刷掉；独立顶层窗口同时符合广播切换台 Program 输出的语义（可拖到第二显示器）。
- 持久化契约与降级：失败一律**非致命**并降级为「仅离屏 + 预监」——无窗口输出能力（非 Windows/未编 D3D11）、句柄或尺寸非法、建链失败三种情况都只让输出视口不可用，预监（`frameReady` JPEG）与离屏合成不受影响；失败被闩存（不逐帧重试）并记录到 `last_output_window_error()`，`OutputWindow` 把原因转成可读文本经 `degraded` 信号上抛为告警日志；`SM_NO_OUTPUT_WINDOW` 可显式跳过装配。headless/CI/RDP/老驱动返回 `nullptr` 属**预期行为**而非缺陷。
- 已知边界：代码链路已闭合，剩余为**人工实机验收**——Windows 真机 `-DSM_BUILD_DESKTOP=ON` 构建后确认输出视口出现、随播放刷新、拖到第二显示器可全屏，且 1080p60 帧率达标（验收步骤见 `docs/BUILD_WINDOWS.md` §7 与 `docs/VERIFY_WINDOWS_MEDIA.md`）。vjfx 引擎仍为 CPU 回退（GPU 着色器属 Phase 3）。

### 4.4 实时预监（PGM）链路

链路由通、帧源待渲染上下文：

- 服务端：`web_gateway.cpp` 的 `/ws/preview` WebSocket 与 `on_pgm_frame`（声明见 `web_gateway.h`）。
- 驱动方：`src/app/main_headless.cpp` 主循环（约 50ms/次）在媒体处于 playing/paused 时以 ~10Hz 调 `capture_pgm_frame_jpeg()`，经 `gateway.on_pgm_frame` 推送；空 JPEG 不推、同帧去重、离开播放态复位。
- 桌面端：`src/app/qt/kernel_host.cpp` 亦调用 `capture_pgm_frame_jpeg()`，经 `frameReady` 信号送 `qml/panels/PreviewStage.qml`（无画面时维持引导空态）。
- 页面侧：`web_ui.cpp` 内嵌页面有 `<img id="pgmPreview">` 接收 `pgm_frame`。
- 剩余缺口：macOS/Linux stub 或 Windows 未启用 `SM_HAS_D3D11` 时 `capture_pgm_frame_jpeg()` 返回空串，故预监要等 P1-2 的 Windows 真分支才有实际画面（见 TASKS_REMAINING P1-3）。

### 4.5 协议层现状

- TCP 适配器：真实实现（`src/protocols/tcp_adapter.cpp`，accept 循环）。
- MIDI 适配器：stub（无 MIDI 库，`midi_adapter.cpp` 有 TODO 未接线分发）。

### 4.6 工程文件容器层（P1-5，2026-09-10 新增）

此前 `src/project/project_file.{h,cpp}` 只管 `manifest.json` 的**结构与语义校验**（`kTopFields[]` 字段表、magic/version/save_mode/media_root/devices 规则、错误码 4001/4002），其头注释明确写着「zip 容器读写（minizip-ng）属平台里程碑」——即第 8 章的**容器读写这一环是真实缺口**。本轮补齐，拆两个层次：

- `src/project/project_zip.{h,cpp}`：自包含 **STORE（不压缩，method 0）** zip 编解码 + CRC-32 + 文件字节 IO 原语（`read_file_bytes`/`write_file_bytes`）。**取舍**：规格书只要求「明文 zip」（任意解压工具可读），未锁定实现库；已压媒体再 deflate 收益极低，引入 minizip-ng/zlib 会把内核里程碑绑死在 vcpkg 上，故以 ~330 行零依赖实现替代，使内核里程碑在 macOS 与 Windows CI 上均可本机自测。写出侧置条目名 UTF-8 标志位（`0x0800`）、DOS 日期固定 `1980-01-01`（同一输入产出**逐字节确定**的包，便于哈希比对与回归）、超 zip32 上限显式报错；读取侧反向搜索 EOCD、逐条回查本地头 `crc`/`size` 并复算 CRC-32，**任一不符即整体失败**（不做部分解析）。
- `src/project/project_archive.{h,cpp}`：§8.3/§8.4 编排。`save_project` = pack 素材入库（`media/<sha256前16位>_<原文件名>`，同名同内容幂等、同名异内容报冲突）→ 写 `saved_at` + `dump(2)` + `validate_manifest` → 写 `.tmp` → **回读 `.tmp` 复验** → `backups/` 轮转（保留最近 5 份）→ 原子 `rename`（跨卷 `copy_file` 兜底），**失败不破坏既有工程**。`load_project` 严格**全有或全无**（失败返回空 manifest，无「半加载」态）。

验证：本机 ctest **26/26**（`test_project_zip` 102 项检查）；另用系统 `unzip -t` / Python `zipfile` 交叉验证互操作性（`No errors detected`、`testzip()==None`、`flag_bits==0x0800`、`compress_type==0`、中文条目名正确解码、清单内 sha256 与条目内容逐位一致）。

### 4.7 M1–M5 的 11.4.2 WP 级 DoD 审计（P1-5 收官，2026-09-10）

按 W1（§5.3 33 op / §5.4 事件 / §5.5 错误码均有代码入口且能经总线收发）、W8（状态机全可达）、W11（§9.5 遥控白名单子集可经 TCP 完成查询与控制、断线不残留半状态）三条逐项对照源码与用例，**共查处 6 处真实缺口并全部补齐**：

- **事件字典分层（`src/core/op_dict.{h,cpp}`）**：`evt_ops_spec()` = §5.4 表核心集 **17** 条（表 16 行，`evt.engine.up`/`down` 同行拆两条）；`evt_ops()` = 核心集 + 实现增补 **5** 条（`evt.playlist.loaded/item_started/item_ended/waiting_go`、`evt.scene.saved`）= **22** 条。此前两者混为一谈，导致硬编码 `17` 的断言与实际全集不符而转红；分层后「规格同源」与「实现超集」不再互相掩盖。`cmd_ops()` = §5.3 全 **33** 条。
- **5 条事件「字典有、代码无」**：`op_dict` 已登记但全仓**无任何 emit 点**，即 W1 不成立。补齐 —— M4 `PlaylistExecutor` 在 `start()`/`advance()`/结束路径按 §5.4 契约载荷上报 `evt.playlist.playing{playlist_id,index,item_id}` 与 `evt.playlist.advance{index,trigger_mode}`；M5 传输面新增 BPM 上报入口 `report_bpm(bpm, confidence)`，含**同值去重**（稳态不刷总线）与 confidence 钳制到 `[0,1]`；内核新增统一入口 `Kernel::pump_heartbeat()` 与 `Kernel::log_event()`。
- **`Kernel::pump_heartbeat()` 的边沿语义**：记心跳 → 推进失联判定 → 按**边沿变化**产出 `evt.engine.up` / `evt.engine.down`（仅在健康态翻转发一条，不是每拍广播）→ 对**在线**引擎逐条产出 `evt.engine.heartbeat{engine_id, load_pct, mem_mb}`，载荷取自可注入的 `Kernel::set_engine_metrics()` 采样器（headless 侧接 `watchdog.status().current_rss_mb`，不额外走 syscall）。语义与 `MsgBus::HeartbeatMonitor`（`kIntervalMs=2000`、`kMaxLost=3`，失联阈值 6000 ms）对齐。
- **headless 心跳 tick 错接（真实缺陷）**：`src/app/main_headless.cpp` 主循环原本直接调 `kernel->heartbeat().tick(now*1000)` + 手工 `note_heartbeat()` 循环。而 `HeartbeatMonitor` 本身**不产生任何事件**（它只维护健康表），故 headless 宿主**永远不会广播 `evt.engine.*`** —— 这是「单测通过但线上不成立」的典型盲区（单测直接调 `pump_heartbeat` 故掩盖了它）。已改为统一走 `kernel->pump_heartbeat()`；Qt 侧 `kernel_host.cpp:161` 本就正确。
- **载荷契约缺字段**：`evt.error` 原只发 `{code, message, module, item_id}`，§5.4 契约为 `{code, msg, source}`；`evt.media.import_done` 原只发 `{total, ok_count, failed_count}`，契约为 `{media_ids:[]}`。均已**补齐契约字段并保留原扩展字段（超集而非替换）**，不破坏既有消费方。
- **§9.5 [1015] 遥控白名单（`is_remote_control_op`）**：放行 `transport.*` / `playlist.*` 前缀族 + `scene.recall` / `sys.set_volume` / `sys.ping` 单例，其余一律按 1002 拒绝。**前缀判定要求 `op.size() > prefix.size()`**，故裸 `"transport."` / `"playlist."` 也被拒绝；非白名单 `sys.shutdown` / `media.*` / `timeline.load` / `scene.save|delete|list` / `remote.hello` / `evt.*` 全覆盖。headless 侧接握手门禁、`[1016]` 观测面转发（`evt.transport.state` / `evt.playlist.*` / `evt.scene.recalled` / `evt.error`；按规格原文**不含** `evt.log` 与 `evt.engine.*`，避免把引擎内部健康面暴露给遥控端）与断线清理（不残留半状态）。
- **零新增告警（11.4.1）**：顺手清掉两处**既有**编译告警 —— `MediaEngine::stop()` 未使用形参 `item_id`（保留形参以与 `play()` 接口对称，标注未使用）、`MediaLibrary::gen_media_id()` 未使用局部量 `ms`。现全仓**编译告警为空**，仅剩既有的 `ld: ignoring duplicate libraries: 'libsm_core.a', 'libsm_engines.a', 'libsm_web.a'` 链接告警（无害）。

钉住手段：`tests/test_op_dict.cpp`（三层计数钉 + 新增 `test_evt_superset()`、`test_remote_whitelist()`）、`tests/test_kernel.cpp`（BusProbe 增 `count()`/`all()`，新增 4 个用例，用例数 19 → **23**）、`src/app/main.cpp`（冒烟程序同步两层计数与白名单双向断言）、`tests/test_media_lib.cpp` / `tests/test_playlist.cpp`（契约载荷断言）。本机 ctest **26/26** 全绿（`test_kernel` 194 项检查）。

## 5. 剩余工作

以 `docs/TASKS_REMAINING.md` 为唯一执行清单：Phase 1 收尾（桌面端真机冒烟、D3D11 窗口输出、PGM 画面闭环、M1–M5 验收审计、冒烟/性能门禁、安装包）→ Phase 2 创作增强 → Phase 3 专业外设 → Phase 4 智能化。全规格完成度估算约 25%，其中 Phase 1 内核约 85%、交付形态 0%。其中 **P1-5 已全部完成**：工程文件容器层（§4.6）与 M1–M5 的 11.4.2 WP 级 DoD 审计（§4.7）均已闭环，P1-4 的「传输接口 501」缺口亦已核实不成立（见 §6）。Phase 1 现剩：P1-1/P1-2 的 Windows 实机人工验收、P1-6 冒烟与性能门禁、P1-7 安装包。

## 6. 文档卫生记录

- 2026-09-10（H-1 / H-2）：核对确认 `README.md`、`docs/BUILD_WINDOWS.md`、`docs/VERIFY_WINDOWS_MEDIA.md` 的 ctest 计数均已为 22/22，无需再改。本文档 §4 按源码重写，作废此前两处过期论断 ——「全仓无 `SM_HAS_D3D11`、无 `d3d11.h`/`dxgi.h` 使用、无 d3d11/dxgi 链接，D3D11 渲染输出属未开始」与「没有调用方驱动 `capture_pgm_frame_jpeg`」；并移除 §3 中已过时的「最近一次全绿 run 基于 commit e21c17b」（其后已有 16 个提交，含 P1-1 桌面端与 CI `desktop` job）。
- 2026-09-10（P1-2 输出窗口）：新增 `test_rhi_geometry`（`compute_ndc_rect` 几何 + `is_valid_output_window_desc` 校验），测试总数 22 → **23**，故全仓 ctest 计数由 `22/22` 统一改为 `23/23`（`README.md`、`docs/BUILD_WINDOWS.md`、`docs/VERIFY_WINDOWS_MEDIA.md`、`docs/TASKS_REMAINING.md` 与本文档 §3）。同时作废 §4.3 中「**无交换链**」与「单视口本地窗口输出（1080p60）尚未落地」两处过期论断 —— D3D11 后端已实现 DXGI 交换链窗口输出。
- 2026-09-10（P1-2 收尾 / `sm_desktop` 接线）：新增 `src/app/qt/output_window.{h,cpp}` 与 `test_output_window_api`，测试总数 23 → **24**，全仓 ctest 计数统一为 `24/24`（同上四处 + 本文档 §3）。同时作废本文档 §4.3 中「窗口输出交换链**尚无调用方接线** —— `sm_desktop` 尚未把 Qt 窗口句柄交给 `create_output_surface()`」这一过期论断：接线条已落地（`QWindow::winId()` → `platform::open_output_window()` → 渲染线程每帧 `present`，失败降级「仅离屏 + 预监」），剩余仅 Windows 实机人工验收。
- 2026-09-10（P1-4 收尾 / 内嵌控制台续播 + `test_web_ui`）：核实 `docs/TASKS_REMAINING.md` 的 P1-4「501 缺口」不成立 —— `main_headless.cpp:185-189`（桌面侧 `kernel_host.cpp:118/123`）本就注入 `transport_pause_fn`/`transport_resume_fn`，`web_gateway.cpp:304-330` 的 501 只是「未注入」兜底分支，故已把 P1-4 由「已核实缺口」改写为「已核实完成（附证据链）」。真实缺口在前端：`src/web/web_ui.cpp` 只有 `sendPause()` 而无恢复入口（而该文件头注释早已宣称「暂停/恢复」），已补 `sendResume()` + `续播` 按钮 + `.btn-resume`。因内嵌页面是**裸 HTML 字符串、编译器零检查**，新增 `tests/test_web_ui.cpp` 做静态一致性断言（页面基础 / `onclick`→`function` 存在性 / 路径→25 条已注册路由白名单 / 四条传输链路端到端），测试总数 24 → **25**，全仓 ctest 计数统一为 `25/25`（`README.md`、`docs/BUILD_WINDOWS.md`、`docs/VERIFY_WINDOWS_MEDIA.md`、`docs/TASKS_REMAINING.md` 与本文档 §3）。
- 2026-09-10（CI 修复 / `<QDebug>`）：commit `f05e31b`（run #34437790755）的 `desktop` job 在步骤 8 `Build sm_desktop` 失败于 `output_window.cpp` 的 `error C2027: use of undefined type 'QDebug'`（详见 §3）。已补 `#include <QDebug>` 并复核 `src/app` 其余日志调用点（`ui_controller.cpp` 同样用 `.noquote()`，但由 QtQml 传递包含，暂无风险 —— **若将来收敛其包含集需一并补头**）。方法学记录：CI job 日志 API 匿名访问为 403 且本机无 `gh`，但**公开的 job 网页（`/actions/runs/<run>/job/<job_id>`）HTML 内含失败注解文本**，可用 `python3` 提取到精确的 MSVC 报错与行号 —— 这是无凭证环境下定位 CI 编译失败的可行路径。该修复已由 run #34438302868（commit `a94c26f`）的 `desktop` job **全绿复验**。
- 2026-09-10（P1-5 工程文件容器层）：新增 `src/project/project_zip.{h,cpp}`、`src/project/project_archive.{h,cpp}` 与 `tests/test_project_zip.cpp`（102 项检查），测试总数 25 → **26**，全仓 ctest 计数统一为 `26/26`（`README.md`、`docs/BUILD_WINDOWS.md`、`docs/VERIFY_WINDOWS_MEDIA.md`、`docs/TASKS_REMAINING.md` 与本文档 §3）。同时**作废 `project_file.h` 头注释长期宣称的「zip 容器读写属平台里程碑」这一过期论断**（该缺口本轮已补齐，新增 §4.6 记录）；`docs/TASKS_REMAINING.md` 的 P1-5 由「现状：待验收核对（勿假设完成）」改写为「容器层已落地并验证 + M1–M5 审计待过」。另注：`sm_core` 已 glob `project/*.cpp`，故新容器层**无需修改根 `CMakeLists.txt`**。
- 2026-09-10（P1-5 收官 / M1–M5 WP 级 DoD 审计）：新增 §4.7 记录本轮 6 处缺口修复；`docs/TASKS_REMAINING.md` 的 P1-5 由「M1–M5 审计待过」升级为「**已全部完成**」，Phase 1 剩余项收窄为「Windows 实机人工验收 + P1-6 门禁 + P1-7 安装包」。测试总数不变（**26**），但 `test_kernel` 用例数 19 → 23、`test_op_dict` 由「硬编码 17」改为三层计数钉。**作废两处过期论断**：(a) `op_dict` 的 `evt_ops()`「就是 §5.4 的 17 条」（实际是核心集 17 + 实现增补 5 = 22，已拆为 `evt_ops_spec()` / `evt_ops()` 两层）；(b) `README.md`/`docs/*` 中「§5.4 事件均已接入总线」的隐含前提（其中 5 条此前无 emit 点）。方法学记录：本轮最有价值的发现来自**跨层核对而非单测** —— `main_headless.cpp` 的心跳 tick 因绕过 `Kernel::pump_heartbeat()` 而永不发 `evt.engine.*`，而新单测因直接调用内核入口而全绿；此类「装配层错接」只能靠「从宿主入口沿调用链核对到事件出口」查出，已作为后续同类审计的固定动作。
