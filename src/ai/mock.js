...
export function mockReply(messages, mode = 'single', context = '') {
  ...
  // 豆包式需求理解对话（离线 Demo）：先思考 → 复述理解 → 澄清 → 邀请确认，不产出完整 PRD
  if (mode === 'chat' || mode === 'align') {
    const thinking =
    ...
    const reply =
      '**我理解你想做**\n\n' +
      '一个能把业务想法快速变成可运行系统的工具。\n\n' +
      '**我注意到**\n\n' +
      '你可能关注的是：需求输入、自动理解、确认后直接出成品。\n\n' +
      '**我想确认**\n\n' +
      '1. 这个系统的核心用户是谁？要解决他们哪一个具体痛点？\n' +
      '2. 你期望的第一版范围大概多大？\n\n' +
      '> 如果你认可以上理解，回复「确认」或点击「✅ 确认并生成成品」，我就开始生成代码并出成品；有偏差请直接指出。\n\n' +
      '> 当前为离线 Mock 理解回复。在「设置」填入 AI key 即可替换为真实模型推理。'
