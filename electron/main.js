...
import { MODE_PROMPTS, DOC_GEN_SYSTEM, DOC_REVISE_SYSTEM, DOC_ANALYZE_SYSTEM, CODE_EDIT_SYSTEM, injectContext } from '../src/ai/prompts.js'
import { mockReply } from '../src/ai/mock.js'
...
function defaultConfig() {
  const rcPath = path.join(__dirname, '..', '.devthinkrc')
  try {
    const rc = JSON.parse(fs.readFileSync(rcPath, 'utf-8'))
    if (rc && typeof rc === 'object') {
      return { baseUrl: 'https://api.deepseek.com/v1', apiKey: '', model: 'deepseek-chat', ...rc }
    }
  } catch {}
...
async function callAI(messages, mode, config, context) {
  if (config?.baseUrl && config?.apiKey) {
    const sys = mode === 'doc' ? DOC_GEN_SYSTEM : mode === 'revise' ? DOC_REVISE_SYSTEM : mode === 'analyze' ? DOC_ANALYZE_SYSTEM : mode === 'edit' ? CODE_EDIT_SYSTEM : MODE_PROMPTS[mode] || MODE_PROMPTS.single
    ...
    return { role: 'assistant', content: data.choices?.[0]?.message?.content || '(空响应)' }
  }
  // 无 key → 离线 Mock 兜底
...
function createWindow() {
  ...
  const devUrl = process.env.ELECTRON_DEV === '1' ? 'http://localhost:5174' : null
  function startLoad() {
